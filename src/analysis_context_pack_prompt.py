...
def normalize_analysis_context_pack_language(report_language: str = "zh") -> str:
    ...
    if normalized == "en":
        return "en"
    if normalized in {"ko", "kr", "korean", "ko-kr", "ko_kr"}:
        return "ko"
    return "zh"
...
def _data_limitation_lines(payload: Dict[str, Any], *, lang: str) -> List[str]:
    ...
    if _has_core_degraded_block(payload):
        if lang == "en":
            ...
            lines.append(
                ...
                "missing, fetch_failed, partial, estimated 상태이면 최종 JSON의 "
                "confidence_level은 높음이면 안 됩니다."
            )
        else:
            lines.append(
    ...
    if lang == "en":
        ...
        lines.append(
            "- 분석 규칙: 보조 데이터 블록 누락은 해당 분석 단락만 제한하며, 누락 자체를 호재나 악재로 해석하지 마세요."
        )
        lines.append(
            "- 안전 규칙: 이 요약의 status, source, warnings, missing_reason만 사용하세요. "
            "raw payload, 뉴스 본문, 추세 원시값, secret, token, webhook은 재현하지 마세요."
        )
    else:
        lines.append(
    ...
    if lang == "ko":
        if phase in INTRADAY_MARKET_PHASES:
            return [
                ...
                "단기 거래 결론 전에 이 한계를 명시하세요."
            ]
        if phase == "premarket":
            return [
                "- 단계/데이터 규칙: 개장 계획은 데이터 신선도 또는 대체 데이터 상태의 제한을 받습니다. "
                "제한된 시세를 오늘의 완성된 가격 흐름처럼 설명하지 마세요."
            ]
        if phase in CONSERVATIVE_MARKET_PHASES:
            return [
                "- 단계/데이터 규칙: 현재 사용 가능한 데이터만 보수적으로 사용하고 존재하지 않는 장중 사실을 채우지 마세요."
            ]
        return []
