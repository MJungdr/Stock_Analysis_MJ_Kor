...
| `MARKET_REVIEW_ENABLED` | Enable the optional market-wide review. Disabled by default so an unset configuration does not add a China (`cn`) market review to `STOCK_LIST` analysis. | `false` |
